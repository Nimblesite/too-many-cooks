...
This repo has the website source under `website/` (Eleventy), generated output in `website/_site/`.
...
- Fix issues at the core where the static content templates are stored — not in the outputted HTML (`website/_site`)
...
Identify the website source files in the repo. Determine the framework (static site generator, Next.js, Hugo, etc.) so you know where to find templates, metadata, and content. (For this repo: Eleventy in `website/`.)
...
Build and run the website locally using `make website-dev` (this repo's command for the Eleventy dev server).
