...
typedef AdminEventHub = ({
  ...
  void Function(String event, Map<String, Object?> payload) pushEvent,
});
...
AdminEventHub createAdminEventHub() {
  ...
  void pushEvent(String event, Map<String, Object?> payload) {
    ...
    for (final entry in [...servers.entries]) {
      consoleError('[TMC] [PUSH] Sending to ${entry.key}');
      unawaited(
        entry.value
            .sendLoggingMessage((
              level: 'info',
            ...
            .then(
              (_) {
                consoleError('[TMC] [PUSH] Sent OK to ${entry.key}');
              },
              onError: (Object e) {
                consoleError('[TMC] [PUSH] FAILED ${entry.key}: $e');
  ...
  return (transports: transports, servers: servers, pushEvent: pushEvent);
}

/// Register admin routes on an Express app.
void registerAdminRoutes(ExpressApp app, TooManyCooksDb db, AdminEventHub hub) {
  // JSON body parser
  ...
  app
    ...
    ..get(
      ...
      handler((req, res) {
        final agents = switch (db.listAgents()) {
          Success(:final value) => value.map(agentIdentityToJson).toList(),
          Error() => <Map<String, Object?>>[],
        };
        final locks = switch (db.listLocks()) {
          Success(:final value) => value.map(fileLockToJson).toList(),
          Error() => <Map<String, Object?>>[],
        };
        final plans = switch (db.listPlans()) {
          Success(:final value) => value.map(agentPlanToJson).toList(),
          Error() => <Map<String, Object?>>[],
        };
        final messages = switch (db.listAllMessages()) {
          Success(:final value) => value.map(messageToJson).toList(),
          Error() => <Map<String, Object?>>[],
        };
    ...
    ..post(
      ...
      handler((req, res) {
        ...
        if (fromAgent == null || toAgent == null || content == null) {
          _sendError(res, 400, 'fromAgent, toAgent, content required');
          return;
        }
        switch (db.adminSendMessage(fromAgent, toAgent, content)) {
          case Success(:final value):
            ...
            res.send(jsonEncode({'sent': true, 'message_id': value}));
          case Error(:final error):
            _sendError(res, 400, jsonEncode(dbErrorToJson(error)));
        }
      }),
...
Map<String, Object?> _parseBody(Request req) {
  ...
  if (dartified is Map) {
    return Map<String, Object?>.fromEntries(
      dartified.entries.map((e) => MapEntry(e.key.toString(), e.value)),
    );
  }
  return {};
}
